...
extern const uint8_t  TXCharacteristicUUID[UUID::LENGTH_OF_LONG_UUID];
extern const uint8_t  RXCharacteristicUUID[UUID::LENGTH_OF_LONG_UUID];

/**
* @class SampleService
* @brief BLE Service to enable ... over BLE
*/
class SampleService {
public:
    /**< Maximum length of data (in bytes) that can be transmitted by the Sample service module to the peer. */
    static const unsigned BLE_SAMPLE_SERVICE_MAX_DATA_LEN = (BLE_GATT_MTU_SIZE_DEFAULT - 3);
...
public:
    ...
    */
    SampleService(BLE &_ble, void (*appCbk)()) :
        ...
        txCharacteristic(TXCharacteristicUUID, sendBuffer, 1, BLE_SAMPLE_SERVICE_MAX_DATA_LEN, GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY),
        rxCharacteristic(RXCharacteristicUUID, receiveBuffer, 1, BLE_SAMPLE_SERVICE_MAX_DATA_LEN,
    ...
    size_t write(const void *_buffer, size_t length) {
        ...
        if (ble.getGapState().connected) {
            ...
            while (length) {
                unsigned bytesRemainingInSendBuffer = BLE_SAMPLE_SERVICE_MAX_DATA_LEN - sendBufferIndex;
                ...
                if ((sendBufferIndex == BLE_SAMPLE_SERVICE_MAX_DATA_LEN) ||
                    // (sendBuffer[sendBufferIndex - 1] == '\r')          ||
                    (sendBuffer[sendBufferIndex - 1] == '\n')) {
                    ble.gattServer().write(getTXCharacteristicHandle(), static_cast<const uint8_t *>(sendBuffer), sendBufferIndex);
                    sendBufferIndex = 0;
                }
            }
        }
...
protected:
    ...
    uint8_t             sendBuffer[BLE_SAMPLE_SERVICE_MAX_DATA_LEN];    /**< The local buffer into which outbound data is
...
#endif /* #ifndef __SAMPLE_SERVICE_H__*/
